...
import * as PURCHASE_RETURN_REPORT_CONFIG from "../config/get_purchase_return_report_config";
import {
    DATE_TIME_FORMATS,
    REPORT_STATUS_TYPES,
    REPORT_TYPES,
...
import { GetPurchaseReturnReportRequest, GetPurchaseReturnReportResponse } from "../dto/report/get_purchase_return_report_dto";
import { PURCHASE_RETURN_QUERIES } from "../queries/get_purchase_return_report_queries";
...
export const getPurchaseReturnReport = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        /* Request Body */
        const body = req.body as GetPurchaseReturnReportRequest;

        /* Inserting into reports table */
        const reportRequestAdded = await db.db
            .insert(reports)
            .values({
                reportName: REPORT_TYPES.purchaseReturnReport,
                companyId: body.companyId,
                status: REPORT_STATUS_TYPES.inProgress,
                fromDateTime: moment.utc(`${body.fromDateTime}`).toDate(),
                toDateTime: moment.utc(`${body.toDateTime}`).toDate(),
        ...
        /* Sending the response */
        res.status(200).json(
            new ApiResponse<GetPurchaseReturnReportResponse>(200, {
                message: "in progress",
            })
        );
        ...
        /* For each worksheet key in config */
        Object.keys(PURCHASE_RETURN_REPORT_CONFIG.WORKSHEETS).forEach((worksheetKey) => {
            /* Worksheet Info */
            const worksheetInfo = PURCHASE_RETURN_REPORT_CONFIG.WORKSHEETS[worksheetKey];
        ...
        try {
            /* Purchase Returns Query */
            const purchaseReturnsQueryPromise = new Promise((resolve, reject) => {
                /* Purchases Query Stream */
                const query = new QueryStream(
                    PURCHASE_RETURN_QUERIES.purchaseReturnsQuery,
                    [body.companyId, body.fromDateTime, body.toDateTime]
                );
            ...
            /* Wait for all promises to resolve */
            await purchaseReturnsQueryPromise;
            ...
            reportLink = await uploadReportFile(
                fileName,
                `${REPORT_TYPES.purchaseReturnReport}-${crypto.randomUUID()}`
            );
